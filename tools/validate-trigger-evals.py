...
def validate_trigger_file(skill_dir: Path) -> int:
    ...
    if not path.exists():
        raise ValidationError(f"{path}: missing trigger eval file")
    ...
    for index, case in enumerate(cases, start=1):
        ...
        if not isinstance(case_dict.get("should_trigger"), bool):
            raise ValidationError(f"{path}: cases[{index}].should_trigger must be boolean")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate trigger evals for APM skills.")
    ...
    for skill_dir in skill_dirs:
        total_cases += validate_trigger_file(skill_dir)
    print(f"validated trigger evals: {len(skill_dirs)} skill(s), {total_cases} case(s)")
    return 0
